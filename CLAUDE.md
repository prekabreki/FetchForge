...
- **Downloader:** yt-dlp — a regular pip dependency (`yt-dlp[default]`, pulled in by `pyproject.toml`), resolved lazily by `get_ytdlp()` → `_resolve_tool()`: bundled `yt-dlp.exe` next to `PKG_DIR` on Windows (optional manual override), else PATH / the interpreter's `sys.prefix/bin` (venv console script) on Linux/macOS.
...
- **Launch:** `pip install fetchforge` then the `fetchforge` command (or `python -m fetchforge`) is the primary flow. From a source clone, `launch.sh` (Linux/macOS) / `launch.bat` (Windows) bootstrap a `.venv`, `pip install -e .`, then run `python -m fetchforge` — the server opens the browser itself once listening.
...
- `launch.bat` — source-clone Windows entry point; opens a visible console window, `pip install -e .` then `python -m fetchforge`; stop the server with Ctrl+C / close the window.
